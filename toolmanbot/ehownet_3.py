...
def searchShortestPath(a, b):
    ...
    for jdx in range(len(aL), idx, -1):
        # print "X:%s"%aL[:jdx]
        L.append(",".join(aL[:jdx]))
    # print "C:%s"%aL[:idx]
    L.append(",".join(aL[:idx]))
    for jdx in range(idx+1, len(bL)+1):
        # print "Y:%s"%bL[:jdx]
        L.append(",".join(bL[:jdx]))
...
class Node(object):
    ...
    def __str__(self):
        return self.data["node_type"]+"(\'"+self.data["name"]+"\')"
    ...
    def isHypernymOf(self, obj):
        ...
        for idx in range(len(pathL)):
            path = pathL[idx]
            level = levelL[idx]
            if self.level == level-1 and len(self.path) < len(path) and self.path == path[:len(self.path)]:
                return True
        return False
    ...
    def isHyponymOf(self, obj):
        ...
        for idx in range(len(pathL)):
            path = pathL[idx]
            level = levelL[idx]
            if self.level == level+1 and len(obj.path) < len(path) and obj.path == path[:len(obj.path)]:
                return True
        return False
    ...
    def isAncestorOf(self, obj):
        ...
        for path in pathL:
            # print(path)
            # print(len(self.path))
            if len(self.path) < len(path) and self.path == path[:len(self.path)]:
                return True
        return False
    ...
    def isDescendantOf(self, obj):
        ...
        pathL = self._getPathList()
        for path in pathL:
            if len(obj.path) < len(path) and obj.path == path[:len(obj.path)]:
                return True
        return False
...
class SemanticTypeNode(Node):
    def __init__(self, tree, data):
        super(SemanticTypeNode, self).__init__(tree, 'semanticType', data)
    ...
    def getDescendantList(self):
        ...
        self.tree.cursor.execute(
            sql, (self.path+',1', self.path+',a', self.level+1))
        L = []
...
if __name__ == "__main__":
    tree = EHowNetTree("./ehownet_ontology.sqlite")
    test1(tree)
    test2(tree)
    test3(tree)
    test4(tree)
